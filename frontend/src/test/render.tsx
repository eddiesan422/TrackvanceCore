...
export function renderApp(ui: ReactElement, options: { permissions?: string[]; path?: string; route?: string } = {}) {
  ...
  const session: Session = {
    user: { id: 'stable-user-id', name: 'Equipo Trackvance', email: 'local@example.test', role: 'ADMIN', permissions: options.permissions ?? ['exports:download', 'artifacts:download', 'runs:execute', 'configurations:write', 'datasets:write', 'exceptions:write', 'exceptions:close'] },
