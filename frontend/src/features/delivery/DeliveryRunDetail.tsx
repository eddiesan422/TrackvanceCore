...
import { Badge, date, Empty, ErrorState, Loading, Notice, number, PageHeading } from '../../components/ui'
...
export function DeliveryRunDetail({ data }: { data: RecordData }) {
  ...
  const awaitingEvidence = data.status === 'SUCCESS' && data.decision === 'COMMITTED' && !data.metrics?.receipt_artifact_id && !pendingRepair
  const [evidencePollingExpired, setEvidencePollingExpired] = useState(false)
  useEffect(() => {
    if (!awaitingEvidence || evidencePollingExpired) return
    // COMMITTED is durable before local publication. Refresh only its projection,
    // for at most one minute; identical/refetched DTOs must not reset the deadline.
    const poll = window.setInterval(() => { void cache.invalidateQueries({ queryKey: ['run', id], exact: true }, { cancelRefetch: false }) }, 1500)
    const deadline = window.setTimeout(() => setEvidencePollingExpired(true), 60_000)
    return () => { window.clearInterval(poll); window.clearTimeout(deadline) }
  }, [awaitingEvidence, evidencePollingExpired, cache, id])
  const receipt = useQuery({ queryKey: ['delivery-receipt', id], queryFn: () => api<RecordData>(`/delivery/runs/${id}/receipt`), enabled: !pending && committed && canDownload && !pendingRepair && !awaitingEvidence })
  ...
  const settling = pending || awaitingEvidence
  const wasPending = useRef(settling)
  useEffect(() => {
    if (wasPending.current && !settling) {
    ...
    wasPending.current = settling
  }, [cache, canAudit, id, settling])
  ...
  return <><PageHeading back="/delivery" eyebrow="DATA DELIVERY / EVIDENCIA DE ENTREGA" title={data.name || 'Entrega de datos'} description={`${data.dataset_name || 'Dataset'} · ${date(data.created_at)}`} action={<><button className="button secondary" disabled={!canDownload || pending || pendingRepair || awaitingEvidence || evidence.isPending} onClick={() => evidence.mutate('manifest')}><FileCheck2 size={16}/> Manifiesto</button><button className="button primary" disabled={!canDownload || !committed || pendingRepair || awaitingEvidence || evidence.isPending} onClick={() => evidence.mutate('receipt')}><Download size={16}/> Receipt</button></>}/>
    ...
    {awaitingEvidence && (evidencePollingExpired ? <Notice>El commit permanece confirmado, pero todavía no se ha confirmado la publicación local de evidencia. La consulta automática se detuvo; actualizar el estado no repite la entrega. <button className="button secondary small" onClick={() => { setEvidencePollingExpired(false); void cache.invalidateQueries({ queryKey: ['run', id], exact: true }) }}>Actualizar estado</button></Notice> : <Loading text="Entrega confirmada. Publicando evidencia local…"/>)}
