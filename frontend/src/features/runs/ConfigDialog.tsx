...
import { ColumnPicker, ComparisonBuilder, NormalizationFields, noNormalization, RuleBuilder, TransformBuilder } from './RuleBuilder'
...
export function ConfigDialog({ module, open, close, initial }: { module: Module; open: boolean; close: () => void; initial?: RecordData }) {
  ...
  const [sentinelRequired, setSentinelRequired] = useState((old.required_columns || []).join(', ')), [keys, setKeys] = useState<string[]>(old.key_columns || []), [nullColumns, setNullColumns] = useState((old.null_columns || []).join(', '))
  ...
  return <Modal open={open} onOpenChange={value => { if (!value && !create.isPending) close() }} title={initial ? `Nueva versión de ${initial.name}` : `Nuevo ${definition.singular}`} description={initial ? `Se publicará una versión nueva. Las ejecuciones y la versión ${initial.version} conservarán su configuración.` : 'Define reglas explícitas y reutilízalas sobre cada nueva versión de tus datos.'} wide><form onSubmit={e => { e.preventDefault(); create.mutate() }}>
    {datasets.isPending ? <Loading/> : datasets.error ? <ErrorState error={datasets.error} retry={() => datasets.refetch()}/> : <div className="form-stack">
      ...
      {module === 'recon' && <>
        ...
        <ColumnMultiSelect label="Columnas clave" hint="La clave compuesta conserva el orden seleccionado y debe existir en ambas fuentes." columns={(schema.data?.columns || []).filter(c => targetSchema.data?.columns.some(t => t.name === c.name))} selected={keys} onChange={setKeys}/>
        <h3>Normalización declarada de claves</h3><NormalizationFields value={normalization} onChange={setNormalization}/>
        ...
        <TransformBuilder label="Transformaciones de origen" value={sourceTransforms} onChange={setSourceTransforms} columns={schema.data?.columns || []}/><TransformBuilder label="Transformaciones de destino" value={targetTransforms} onChange={setTargetTransforms} columns={targetSchema.data?.columns || []}/>
      ...
      {module === 'intake' && <TransformBuilder value={transforms} onChange={setTransforms} columns={schema.data?.columns || []}/>}
