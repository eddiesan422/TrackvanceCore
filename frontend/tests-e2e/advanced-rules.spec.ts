...
test('Recon concilia N:1 con transformación explícita y SUM/COUNT independientes', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'Nuevo control', exact: true }).first().click()
  const dialog = page.getByRole('dialog')
  ...
  await transforms.getByLabel('Separador decimal', { exact: true }).fill(',')
  await transforms.getByLabel('Separador de miles', { exact: true }).fill('.')
